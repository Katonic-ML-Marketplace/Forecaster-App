...
@app.route('/app/')
def index():
    return render_template('build-forecast-v3.html',app_route=os.getenv("ROUTE")) # Application
...
def forecast_settings(message):
    ...
    # Initial forecast settings - the first time the user sends forecast settings through the app - will use this value in forecaster method
    ...
    # Use Facebook Prophet through forecaster method
    forecast = forecaster(time_series_data, forecast_settings, column_headers, freq, build_settings)
...
def update_chart(message):
    ...
    ### Setup variables for use in the forecaster method
